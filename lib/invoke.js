...
function invoke(func, data, funcsDesc, options) {
  ...
  const namespace =
    desc.namespace || opts.namespace || helpers.getDefaultNamespace(config);
    ...
    _.each(desc.sequence.slice(), (sequenceFunction) => {
      if (promise) {
        promise = promise.then((result) =>
          invoke(sequenceFunction, result.body, funcsDesc, opts)
        );
      } else {
    ...
    const parseReponse = (err, response) => {
      if (err) {
        ...
        if (response.statusCode !== 200) {
          if (response.body && response.body.stack && response.body.message) {
            reject(deserializeError(response.body));
          } else {
    ...
    core.ns.services.get((err, servicesInfo) => {
      if (err) {
        ...
        const functionService = _.find(
          servicesInfo.items,
          (service) =>
            service.metadata.labels && service.metadata.labels.function === func
        );
        if (_.isEmpty(functionService)) {
          opts.log(`Not found any information about the function "${func}"`);
        }
        const port =
          functionService.spec.ports[0].name ||
          functionService.spec.ports[0].port;
        const url = `${APIRootUrl}/api/v1/namespaces/${namespace}/services/${func}:${port}/proxy/`;
        const invokeConnectionOptions = Object.assign(connectionOptions, {
          url,
          headers: {
        ...
        if (_.isEmpty(requestData)) {
          ...
          request.post(
            Object.assign(invokeConnectionOptions, requestData),
            parseReponse
          );
        }
      }
